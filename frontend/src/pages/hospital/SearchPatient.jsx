  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} style={{ maxWidth: '600px', margin: '0 auto' }}>
      <div className="glass-panel text-center">
        <h2 className="heading-gradient" style={{ marginBottom: '2rem' }}>Search Patient Directory</h2>
        ...
        {patient && (
          <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="glass-panel" style={{ background: 'rgba(0, 242, 254, 0.05)', textAlign: 'left', border: '1px solid var(--secondary-color)' }}>
            <h3 style={{ color: 'var(--secondary-color)', marginBottom: '1rem' }}>Patient Found!</h3>
            <p><strong>Name:</strong> {patient.name}</p>
            <p><strong>Phone:</strong> {patient.phone?.substring(0, 3)}XXXXXXX</p>
            <p><strong>Health ID:</strong> {patient.healthId}</p>
            <p style={{ marginTop: '1rem', fontSize: '0.8rem', color: 'var(--text-secondary)' }}>Note: This patient has been added to your hospital directory.</p>
          </motion.div>
        )}
      </div>
    </motion.div>
